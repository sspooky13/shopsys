...
class ProductSearchRepository
{
     ...
     * @return int[]
     */
    public function getFoundProductIds(int $domainId, string $query): array
    {
        if (!$this->tsqueryFactory->isValidSearchText($query)) {
            return [];
        }
        ...
        $sql = '
          ...
          ORDER BY relevance';
        $parameters = [
            '%' . DatabaseSearching::getLikeSearchString($query) . ' %',
            '%' . DatabaseSearching::getLikeSearchString($query),
            $this->tsqueryFactory->getTsqueryWithAndConditions($query),
            $this->tsqueryFactory->getTsqueryWithOrConditions($query),
            DatabaseSearching::getFullTextLikeSearchString($query),
            $this->tsqueryFactory->getTsqueryWithAndConditionsAndPrefixMatchForLastWord($query),
            $this->tsqueryFactory->getTsqueryWithOrConditionsAndPrefixMatchForLastWord($query),
            $domainId,
        ];
